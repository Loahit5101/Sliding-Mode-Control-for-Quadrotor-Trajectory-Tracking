     ...
     # obtain the desired values by evaluating the corresponding trajectories
        ...
        if self.t<=65:
            ...
            if s1>0:
                ...
                sgns1=0
            # U1 equation
            u1 = m * (g - lamda1 * (zdot - self.zdot_des) + self.zddot_des - rhok1 * sgns1) * (1/(np.cos(roll_ang)*np.cos(pitch_ang)))
...
# odometry callback function (DO NOT MODIFY)
  ...
  def save_data(self):
        ...
        with open("/home/loahit/rbe502_project/src/control/scripts/log.pkl", "wb") as fp:
